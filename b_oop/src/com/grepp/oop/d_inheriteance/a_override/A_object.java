...
// 메서드 내에서 다른 클래스이 메서드를 호출 : use-a관계
// 강아지는 동물이다 ( is -a )
// 원은 점을 가지고 있다 ( has a )
// ~는~다 이거라 ~가지고 있다 / is-a는 하나에 속한다는거고 hase-a는 이거를 가지고 있다
//원은 점들의 집합, 원은 점이다 관계가 아니고 원은 점을 가지고 있는 has-a 관계.
public class A_object {
    ...
    // Object class의 toString은 full qualified name + '@' + hashcode를 반환하지만
    // A_Object 클래스의 toString은 이름과 설명에 대한 내용으로 반환하고 싶다.

    // NOTE DA02 : override
    // override : 재정의, 부모클래스로 부터 상속 받은 메서드를 자식클래스에서 재정의하는 것
    //            반환타입, 매개변수, 이름이 일치해야한다.
    //            접근제한다는 더 넓은 범위로만 재정의 할 수 있다.
    //            protectecd -> public (o)  public -> protected (x)
    ...
    // 동등성 : 객체의 속성에 담긴 값이 같으면 같은 객체로 판단(꼭 모든 객체가 같을 필요는 없음)


    @Override
    public boolean equals(Object o) {
        if (!(o instanceof A_object aObject)) return false;
